...
When updating page content, multiple locations must stay in sync. Missing even one creates inconsistencies between visible text, search engine metadata, and structured data. Use this checklist alongside `Agent(content-designer)`, `Agent(seo-engineer)`, and `content-creation` skill.
...
- **Roles**: `Agent(content-designer)` (content changes), `Agent(seo-engineer)` (metadata, structured data), `Agent(frontend-engineer)` (implementation)
