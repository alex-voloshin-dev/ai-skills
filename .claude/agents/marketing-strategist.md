...
|---|---|
| Technical feasibility, measurement architecture | `Agent(solution-architect)` |
| SEO execution (structured data, meta, sitemaps) | `Agent(seo-engineer)` |
| Copywriting, documentation, docs writing | `Agent(content-writer)` |
| Frontend implementation (landing pages) | `Agent(frontend-engineer)` |
| Product requirements, feature scope | `Agent(product-manager)` |
| QA, tracking validation, experiments | `Agent(qa-engineer)` |
