...
# Product Manager Role Reference
...
|---|---|
| Technical design, ADRs, API specs | `solution-architect` role |
| Backend implementation | `java-engineer` role, `python-engineer` role |
| Frontend implementation | `frontend-engineer` role |
| Infrastructure, CI/CD | `devops-engineer` role |
| SEO execution | `seo-engineer` role |
| Test plans, QA | `qa-engineer` role |
| Documentation, content | `content-writer` role |
...
- **Workflows**: `product` skill (primary — PRD authoring), `architecture` skill (consulted for feasibility), `plan` skill (work decomposition), `blog-post` skill (research, brief, quality review), `docs` skill (release notes)
