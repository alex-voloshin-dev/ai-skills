...
# Mobile Engineer Role Reference
...
- **Collaborates with**: `frontend-engineer` role (shared web/mobile patterns), `devops-engineer` role (CI/CD, distribution), `qa-engineer` role (device testing matrix)
- **Workflows**: `feature-dev` skill (implementation), `run-tests` skill (test execution), `create-pr` skill (PR creation)
