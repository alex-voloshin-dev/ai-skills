...
# Software Engineer Role Reference
...
|---|---|---|
| Frontend / UI | `*.tsx`, `*.jsx`, `*.ts` (components/pages), `*.css`, `*.scss` | `frontend-engineer` role |
| Java backend | `*.java`, `pom.xml`, Spring configs | `java-engineer` role |
| Python backend | `*.py`, `requirements.txt`, `pyproject.toml` | `python-engineer` role |
| Infrastructure | `*.tf`, `Dockerfile`, `Helm`, `docker-compose.*`, CI/CD | `devops-engineer` role |
| Database | `*.sql`, migration files, schema definitions | `db-engineer` role or stack-specific role |
| ML / AI | Model configs, training scripts, notebooks | `ml-engineer` role |
| Mobile | `*.swift`, `*.kt`, `*.dart`, platform configs | `mobile-engineer` role |
| Data pipelines | ETL scripts, DAGs, dbt models, Spark jobs | `data-engineer` role |

**Non-engineering roles** (`product-manager` role, `marketing-strategist` role, `content-writer` role, `content-designer` role, `ui-ux-designer` role, `solution-architect` role, `system-architect` role, `prompt-engineer` role) **NEVER modify source code, configs, infrastructure, or dependency files.** They produce documentation, specifications, and design artifacts only.
...
- **Extended by**: `java-engineer` role, `python-engineer` role, `frontend-engineer` role, `devops-engineer` role, `db-engineer` role, `system-architect` role, `cloud-architect` role, `devops-architect` role, `ui-ux-designer` role, `content-designer` role, and all other Layer 2 roles
- **Workflows**: `feature-dev` skill, `bugfix` skill, `pre-commit` skill, `code-review` skill, `security-scan` skill
