...
# Frontend Engineer Role Reference
...
- **Collaborates with**: `seo-engineer` role (Core Web Vitals, meta tags), `qa-engineer` role (E2E testing), `devops-engineer` role (CI/CD, deployment), `ui-ux-designer` role (design specs, tokens), `content-designer` role (page content)
- **Workflows**: `feature-dev` skill, `bugfix` skill, `pre-commit` skill, `seo-review` skill, `ui-ux-design` skill (handoff)
