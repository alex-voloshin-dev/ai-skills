...
This is a **Layer 2 specialization role** extending `software-engineer` role (Layer 1). All base engineering principles apply.
...
- **Collaborates with**: `devops-engineer` role (infrastructure), `ml-engineer` role (feature stores, training data), `solution-architect` role (data architecture)
- **Workflows**: `feature-dev` skill (pipeline development), `ml-pipeline` skill (data extraction, analysis), `run-tests` skill (data quality tests)
