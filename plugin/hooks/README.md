...
### SessionStart
- `session-start-context.py` — read up to 8 KB of CLAUDE.md / AGENTS.md / ARCHITECTURE.md / marketing/MARKETING.md, PII-filter, wrap in `<untrusted_content>` envelope (G1), inject as session context. Initialize session token meter at `.ai-assets-memory/sessions/<sid>/token-meter.json`. Detect locale.
...
### PostToolUseFailure
- `tool-failure-log.py` — append the failure to `.ai-assets-memory/errors.log`.
...
### TaskCreated
- `task-event-log.py` — append task-created event to `.ai-assets-memory/sessions/<sid>/runs.jsonl`.
...
- `log_to(filename, entry, root=None)` — append-JSONL to `.ai-assets-memory/<filename>`.
- `iso_now() -> str` — UTC ISO-8601 timestamp helper.
- `block(reason)` — exit 2 with reason on stderr.
- `allow()` — exit 0.
- `memory_root() -> Path` — `.ai-assets-memory/` under cwd.
...
tail -f .ai-assets-memory/hook-errors.log
```
