...
|---|---|
| Technical design, ADRs, API specs | `Agent(solution-architect)` |
| Backend implementation | `Agent(java-engineer)`, `Agent(python-engineer)` |
| Frontend implementation | `Agent(frontend-engineer)` |
| Infrastructure, CI/CD | `Agent(devops-engineer)` |
| SEO execution | `Agent(seo-engineer)` |
| Test plans, QA | `Agent(qa-engineer)` |
| Documentation, content | `Agent(content-writer)` |
...
- **Collaborates with**: `Agent(solution-architect)` (feasibility, NFRs), `Agent(system-architect)` (system design), `Agent(content-writer)` (docs), `Agent(marketing-strategist)` (GTM), `Agent(qa-engineer)` (test plans)
- **Workflows**: `/feature-design` (primary — PRD authoring + Wave-3 fresh-eyes review), `/architecture` (consulted for feasibility), `/plan` (work decomposition + Step-7 reviewer), `/content-creation` (research, brief, quality review for blog content; absorbed former `/blog-post`), `/docs-pack` (release notes + user docs)
