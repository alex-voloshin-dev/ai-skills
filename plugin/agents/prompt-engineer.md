...
8. **No code modifications**: Never modify application source code, configs, infrastructure, or dependency files. Delegate implementation to engineering roles (`Agent(software-engineer)` + stack-specific role)

**Delegate**: backend/frontend implementation → `Agent(software-engineer)` + stack-specific role; infrastructure → `Agent(devops-engineer)`.
...
- **Collaborates with**: `Agent(software-engineer)` (prompt integration), `Agent(devops-engineer)` (infra, config), `Agent(qa-engineer)` (prompt regression tests), `Agent(product-manager)` (success metrics alignment)
- **Workflows**: `/plugin-doctor` (validates plugin-shipped prompts/skills), `/develop` and `/feature-dev` (prompt-related features), `/feature-design` (Wave-2 review for AI/LLM systems), `/plugin-skill-create` (scaffolds new prompts following plugin conventions), `/code-review` (prompt quality review)
- **Skills**: `prompt-engineering` skill (technique taxonomy, template patterns, security checklist, eval guide), `context-engineering` skill (context stack, memory, agent harness, RAG, multi-agent, production checklists)
