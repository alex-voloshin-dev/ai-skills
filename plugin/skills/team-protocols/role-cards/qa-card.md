...
2. **No flaky tests.** Every test you write must be deterministic. Sleep-based waits are forbidden — use explicit waits.
...
3. Verify or write tests for happy path, edge cases, error states, boundary values. Use existing test patterns from the project's test directory; do NOT introduce a new framework.
...
- `result.files_changed` — test files you created or modified (empty array is fine for verify-only)
...
- No application-code edits — your `Edit` / `Write` is for test files and fixtures only.
