...
Automated browser-based visual audit of existing or newly implemented UI. Use alongside `Agent(ui-ux-designer)` role and `ui-ux-design` skill. Referenced by `/ui-ux-design` workflow (Step 3a and Step 11).
...
| Fix | Delegate To |
|---|---|
| Layout/CSS fixes | `Agent(frontend-engineer)` |
| Content/copy changes | `Agent(content-designer)` |
| Accessibility fixes | `Agent(frontend-engineer)` + `Agent(ui-ux-designer)` |
| SEO issues | `Agent(seo-engineer)` |
| Design system gaps | `Agent(ui-ux-designer)` |
```
...
7. **Flag deviations**: file as implementation fixes for `Agent(frontend-engineer)`
