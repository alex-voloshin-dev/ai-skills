...
Best practices for Figma project structure, component organization, design token management, and developer handoff. Use alongside `Agent(ui-ux-designer)` role and `ui-ux-design` skill.
...
```
[Project Name]/
├── 01 Foundations          # Design tokens and primitives
│   ├── Colors
│   ├── Typography
│   ├── Spacing & Grid
│   ├── Icons
│   └── Elevation & Effects
├── 02 Components           # Reusable component library
│   ├── Atoms
│   ├── Molecules
│   └── Organisms
├── 03 Templates            # Page-level layout templates
│   ├── Marketing Pages
│   ├── App Pages
│   └── Error Pages
├── 04 Screens              # Actual designs per feature/page
│   ├── [Feature Name]
│   │   ├── Desktop
│   │   ├── Tablet
│   │   └── Mobile
│   └── [Feature Name]
├── 05 Prototypes           # Interactive prototypes
│   ├── User Flow A
│   └── User Flow B
└── 06 Documentation        # Design specs and guidelines
    ├── Design Principles
    ├── Accessibility Guide
    └── Handoff Notes
```
...
|---|---|---|
| Pages | Numeric prefix + Title Case (preserves explicit ordering in Figma's left rail) | `02 Components` |
