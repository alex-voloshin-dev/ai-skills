---
name: analyze-local
description: Analyze local Docker environment — collect container status, logs, networking, resource usage, and diagnose issues. Applies SRE or DevOps roles for investigation. Use when a local container won't start, a service is unreachable from the host, docker-compose stack is misbehaving, resource usage is unexpected, or as the diagnosis step of a local-environment bugfix flow.
context: fork
