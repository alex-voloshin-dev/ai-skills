...
|---|---|---|
| Frontend / UI | `*.tsx`, `*.jsx`, `*.ts` (components/pages), `*.css`, `*.scss` | `@frontend-engineer` |
| Java backend | `*.java`, `pom.xml`, Spring configs | `@java-engineer` |
| Python backend | `*.py`, `requirements.txt`, `pyproject.toml` | `@python-engineer` |
| Infrastructure | `*.tf`, `Dockerfile`, `Helm`, `docker-compose.*`, CI/CD | `@devops-engineer` |
| Database | `*.sql`, migration files, schema definitions | `@db-engineer` or stack-specific role |
| ML / AI | Model configs, training scripts, notebooks | `@ml-engineer` |
| Mobile | `*.swift`, `*.kt`, `*.dart`, platform configs | `@mobile-engineer` |
| Data pipelines | ETL scripts, DAGs, dbt models, Spark jobs | `@data-engineer` |

**Non-engineering roles** (`@product-manager`, `@marketing-strategist`, `@content-writer`, `@content-designer`, `@ui-ux-designer`, `@solution-architect`, `@system-architect`, `@prompt-engineer`) **NEVER modify source code, configs, infrastructure, or dependency files.** They produce documentation, specifications, and design artifacts only.
