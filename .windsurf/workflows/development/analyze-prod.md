...
**Cloud platform detection**: Read `AGENTS.md` to identify which cloud platform is used. Consult `cloud-platforms` skill and load the corresponding reference module (GCP, Azure, or AWS) for platform-specific CLI commands.
...
Detect platform from `AGENTS.md` and verify authentication. Consult `cloud-platforms` skill for platform-specific auth verification commands:
