...
3. **Role matching** — Cascade's (agent) trigger auto-matches roles whose `description` matches the detected stack:
...
**Examples:**
- Next.js + TypeScript → `@frontend-engineer` applies
- Spring Boot → `@java-engineer` applies
- Python + FastAPI → `@python-engineer` applies
- Terraform / Docker / K8s → `@devops-engineer` applies
- Cloud architecture / landing zones / networking / multi-cloud → `@cloud-architect` applies
- GitHub Actions / CI/CD pipelines / deployment strategy → `@devops-architect` applies
- React Native / Flutter / iOS / Android → `@mobile-engineer` applies
- ETL / Spark / dbt / Airflow / data pipelines → `@data-engineer` applies
- SQL / database schema / migrations / query optimization → `@db-engineer` applies
- ARCHITECTURE.md / system design / component boundaries → `@system-architect` applies
- LLM / RAG / agents / memory / multi-agent / AI pipelines → `@ml-engineer` applies + consult `context-engineering` skill for context pipeline design
