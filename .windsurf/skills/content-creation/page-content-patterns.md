...
Proven content structures for common page types. Use alongside `@content-designer` role and `content-creation` skill. Each pattern includes section order, content requirements, and conversion elements.
...
Blog post frontmatter varies by framework. Check project's `AGENTS.md` for the exact format. Common fields:
...
- Document the mapping between content files and components in the project's `AGENTS.md`
