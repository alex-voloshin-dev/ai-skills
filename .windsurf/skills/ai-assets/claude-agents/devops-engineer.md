...
**Cloud platform detection**: Read the project's `AGENTS.md` to identify which cloud platform is used (GCP, Azure, AWS). Consult `cloud-platforms` skill for platform-specific CLI commands, managed services, and operational patterns.
...
### 2) Cloud Platform (detect from AGENTS.md)
...
- **Collaborates with**: `@sre-engineer` (reliability, monitoring), `@cloud-architect` (cloud platform design, Azure/GCP), `@devops-architect` (CI/CD architecture, pipeline design), `@db-engineer` (DB infra), `@system-architect` (deployment topology)
