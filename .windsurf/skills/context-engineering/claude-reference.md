...
- Non-AI software engineering tasks → use `@software-engineer` + stack-specific role
...
| 2. Developer instructions | Role definitions, hard rules, reasoning protocols, conventions | Maps to: Cascade rules ((auto-loaded), (agent)) |
...
| 4. Runtime state | Current project context, AGENTS.md, active file, task state, structured state blob | Maps to: Cascade AGENTS.md files |
...
| File | Contents | Guide §§ |
|---|---|---|
| `context-stack-model.md` | 8-layer architecture, layer design patterns, position effects, mapping to Cascade assets | §3 |
...
- **Follows rules**: `@prompt-engineer` (prompt-level techniques, security, eval-first quality)
...
- **Collaborates with roles**: `@solution-architect` (AI system design), `@product-manager` (agent contracts), `@sre-engineer` (context observability), `@software-engineer` (implementation)
