...
Projects should have a `TESTING.md` at the root (and per-service in monorepos) documenting test infrastructure, commands, credentials, and organization. Created by `/project-init` using `@qa-engineer`. Always read `TESTING.md` before writing or running tests.

## Integration

- **Follows rules**: `@qa-engineer` (test strategy, automation, local test infra), `@software-engineer` (code quality)
